...
class TranslatableTest extends PluginTestCase
{
    ...
    {
        Relation::morphMap([
            'morph.key.trait' => CountryModel::class,
        ]);
    ...
    public function testHasTranslatableAttributes()
    {
        $obj = new CountryModel();
        $this->assertTrue($obj->hasTranslatableAttributes());
        $this->assertEquals(['name', 'states'], $obj->getTranslatableAttributes());
    }

    public function testLangChaining()
    {
        $this->recycleSampleData();

        $obj = CountryModel::first();
        $obj->lang('fr')->name = 'Australie';
        $obj->save();
